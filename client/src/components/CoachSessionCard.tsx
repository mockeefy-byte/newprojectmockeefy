...
export default function CoachSessionCard() {
  ...
  const allProfiles = useMemo<MentorProfile[]>(() => {
    ...
    return rawExperts.map((expert: any) => {
      ...
      const displayPrice = typeof expert.price === "number" && expert.price > 0 ? `\u20B9${expert.price}` : "\u20B9\u2014";
  ...
  const [categoryFilter, setCategoryFilter] = useState<string>("IT"); // Default to main category
  ...
  const categorizedProfiles = useMemo(() => {
    ...
    // Only show the main category (IT) on the homepage
    const mainCategory = "IT";
    const mainCategoryData = categoriesData.find((c: any) => c.name === mainCategory && c.status !== 'Inactive');
    
    if (mainCategoryData) {
      const sectionProfiles = filteredProfiles.filter(p => {
        const lowerTitle = mainCategory.toLowerCase();
      ...
      if (sectionProfiles.length > 0) {
        ...
        sections.push({ title: mainCategory, profiles: sorted });
      }
    }
      ...
      <div className="w-full bg-white border border-slate-200/80 rounded-2xl p-4 md:p-5 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
          ...
          >
            ...
            Filters
            {(experienceFilter !== "Any" || locationFilter !== "Any") && (
