...
const isValidGoogleMeetLink = (link: string) => /^https?:\/\/meet\.google\.com\/[a-z0-9-]+/i.test(link.trim());

const hasBookableSlots = (weekly: Record<string, Slot[]> = {}) => {
  return Object.values(weekly).some((slots) =>
    Array.isArray(slots) && slots.some((slot) => slot?.from && slot?.to)
  );
};

const validateWeeklySlots = (weekly: Record<string, Slot[]> = {}) => {
  for (const [day, slots] of Object.entries(weekly)) {
    ...
    for (let i = 0; i < sortedSlots.length; i += 1) {
      ...
      if (!slot.from || !slot.to) {
        return `${dayLabel[day] || day}: choose both start and end time.`;
      }
      if (slot.from >= slot.to) {
        return `${dayLabel[day] || day}: end time must be after start time.`;
      }
      const previous = sortedSlots[i - 1];
      if (previous && slot.from < previous.to) {
        return `${dayLabel[day] || day}: time slots cannot overlap.`;
      }
    }
  }
...
const ExpertAvailability = () => {
  ...
  const saveAvailability = async () => {
    try {
      ...
      const slotError = validateWeeklySlots(payload.weekly || {});
      if (slotError) {
        toast.error(slotError);
        return;
      }
      if (hasBookableSlots(payload.weekly || {}) && !link) {
        toast.error("Add a Google Meet link before saving bookable time slots.");
        return;
      }
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Required when you add bookable time slots. New bookings will automatically show this Meet link.
                  </p>
                </div>
                <div>
