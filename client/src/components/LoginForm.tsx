...
export const LoginForm = () => {
  ...
  useEffect(() => {
    ...
    if (params.get('session') === 'expired') {
      ...
      setError('Please log in to continue. If you don\'t have an account, create one now.');
    }
  }, [location.search]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (errorMsg.includes("Invalid credentials")) {
        setError("Email or password is incorrect. Don't have an account? Create one now.");
      } else if (errorMsg.includes("Account not found")) {
        setError("Account not found. Please create an account to get started.");
  ...
  const handleGoogleSuccess = async (accessToken: string) => {
    ...
    try {
      ...
      if (result?.success && result?.user) {
        ...
        setError("Account not found. Please create a new account or contact support.");
      }
    } catch (err: any) {
