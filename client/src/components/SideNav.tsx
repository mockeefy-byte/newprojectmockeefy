...
export default function SideNav({ isOpen = false, onClose, className = "" }: SideNavProps) {
  ...
  const navItems = [
    ...
    { id: "earnings", icon: DollarSign, label: 'Earnings', to: '/dashboard/earnings' },
    { id: "reports", icon: FileText, label: 'Reports', to: '/dashboard/reports' },
  ];

  const SidebarContent = () => (
    <aside className={`w-full h-full border-r border-slate-200 bg-white flex flex-col shrink-0 ${className}`}>
