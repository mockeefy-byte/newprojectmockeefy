...
const isValidGoogleMeetLink = (link?: string | null) => /^https?:\/\/meet\.google\.com\/[a-z0-9-]+/i.test(String(link || '').trim());

const extractMeetingLink = (session: any) => {
  const link = session?.meetingLink || session?.meetLink || session?.data?.meetingLink || session?.data?.meetLink || null;
  return isValidGoogleMeetLink(link) ? String(link).trim() : null;
};
...
const GoogleMeetOnlyMeeting = ({
  ...
  useEffect(() => {
    ...
    const fetchSession = async () => {
      try {
        ...
        const link = extractMeetingLink(res.data);
        if (link) {
          setFetchedMeetingLink(link);
          setMeetLinkInputValue((current) => current || link);
        }
      } catch (_) {}
      finally {
  ...
  const googleMeetLink = localMeetLink || fetchedMeetingLink || extractMeetingLink(resolvedSession) || extractMeetingLink(sessionData?.session);
  const isGoogleMeetUrl = isValidGoogleMeetLink(googleMeetLink);
  ...
  const handleSaveMeetLink = async () => {
    ...
    try {
      const res = await axios.patch(`/api/sessions/${meetingId}/meeting-link`, { meetingLink: link });
      const updatedSession = res.data?.data || res.data;
      const savedLink = extractMeetingLink(updatedSession) || link;
      setResolvedSession((prev: any) => ({ ...(prev || {}), ...(updatedSession || {}), meetingLink: savedLink }));
      setFetchedMeetingLink(savedLink);
      setLocalMeetLink(savedLink);
      setShowMeetLinkInput(false);
      setMeetLinkInputValue(savedLink);
      toast.success('Google Meet link saved. Opening now...');
  ...
  const fetchMeetingLinkOnce = async () => {
    ...
    try {
      ...
      if (link) {
        setFetchedMeetingLink(link);
        setMeetLinkInputValue(link);
        toast.success('Meet link loaded. Opening now...');
      ...
      <div className="max-w-5xl mx-auto space-y-5">
        ...
        <div className="rounded-2xl bg-white border border-slate-200 p-5 sm:p-6 shadow-sm">
          <div className="flex flex-col gap-5">
            ...
            {isGoogleMeetUrl ? (
              <div className="flex flex-wrap items-center gap-3">
                <span className="text-sm text-emerald-700 bg-emerald-50 border border-emerald-200 px-2.5 py-1 rounded-md">
                  Link saved. You can join now.
                </span>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={handleSaveMeetLink} disabled={savingMeetLink} className="px-4 py-3 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-medium disabled:opacity-50">
                    {savingMeetLink ? 'Saving...' : 'Save & Join'}
                  </button>
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button type="button" onClick={fetchMeetingLinkOnce} className="px-4 py-3 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200">
                  Refresh link
                </button>
                ...
                <span className="text-sm text-slate-500">Create a meeting, copy the link, then paste and save it for this session.</span>
              </div>
            ) : (
              <div className="flex flex-wrap items-center gap-3">
                <span className="text-slate-600">Waiting for expert to add the Google Meet link.</span>
                <button type="button" onClick={fetchMeetingLinkOnce} className="px-4 py-3 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200">
                  Refresh link
                </button>
              </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-4 sm:p-5 shadow-sm">
          ...
          <div className="space-y-2.5 text-sm text-slate-600">
            {role === 'expert' ? (
              ...
              <>
                <p className="flex items-start gap-2"><CheckCircle2 className="w-4 h-4 text-emerald-600 mt-0.5 shrink-0" />Wait for the expert's saved Google Meet link.</p>
                <p className="flex items-start gap-2"><CheckCircle2 className="w-4 h-4 text-emerald-600 mt-0.5 shrink-0" />Click "Open in Google Meet" once the link appears.</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/45" onClick={() => setShowMeetHelp(false)}>
          <div className="bg-white rounded-2xl max-w-md w-full p-5 border border-slate-200 shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="text-slate-600 text-sm space-y-3">
              ...
              <p><strong className="text-slate-900">Candidate:</strong> Wait for the expert's saved link, then click "Open in Google Meet". Use "Refresh link" if it does not appear yet.</p>
            </div>
...
export default function LiveMeetingPage() {
  ...
  const role = searchParams.get('role') || (user as any)?.role || (user as any)?.userType?.toLowerCase() || (location.state as any)?.role;
