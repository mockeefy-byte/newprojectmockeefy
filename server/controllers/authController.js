      ...
      const accessToken = jwt.sign(
        {
          ...
          userType: normalizeUserType(existingUser.userType),
...
export const registerUser = async (req, res) => {
  ...
  try {
    ...
    const accessToken = jwt.sign(
      {
        email: newUser.email,
        userType: normalizeUserType(newUser.userType),
        name: newUser.name,
        userId: newUser._id
      },
      JWT_SECRET,
...
export const loginUser = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res.status(401).json({ message: "Account not found. Please create an account to get started." });
    }
...
export const authenticateToken = (req, res, next) => {
  ...
  jwt.verify(token, JWT_SECRET, (err, user) => {
    ...
    req.user = user;
    req.user.userType = normalizeUserType(req.user.userType);
    next();
  });
};
