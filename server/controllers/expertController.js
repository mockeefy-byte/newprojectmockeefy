...
const hasBookableSlots = (weekly = {}) => {
  return Object.values(weekly || {}).some((slots) =>
    Array.isArray(slots) && slots.some((slot) => slot?.from && slot?.to)
  );
};

const validateWeeklySlots = (weekly = {}) => {
  for (const [day, slots] of Object.entries(weekly || {})) {
    ...
    for (let i = 0; i < sortedSlots.length; i += 1) {
      ...
      if (!from || !to) return `${dayLabels[day] || day}: choose both start and end time.`;
      if (!/^\d{2}:\d{2}$/.test(from) || !/^\d{2}:\d{2}$/.test(to)) return `${dayLabels[day] || day}: invalid time format.`;
      if (from >= to) return `${dayLabels[day] || day}: end time must be after start time.`;

      const previous = sortedSlots[i - 1];
      if (previous && from < String(previous.to || "")) return `${dayLabels[day] || day}: time slots cannot overlap.`;
    }
  }
...
export const uploadVerificationDocs = async (req, res) => {
  try {
    ...
    // Handle uploaded files (Cloudinary or local)
    ...
    if (req.body.linkedin && req.body.linkedin.trim()) {
      expert.verification.linkedin = req.body.linkedin.trim();
    }
...
export const updateAvailability = async (req, res) => {
  try {
    ...
    // Auto-create expert profile if it doesn't exist
    if (!expert) {
      expert = new ExpertDetails({ userId: queryUserId });
      await expert.save();
    }
    ...
    const nextWeekly = newAvailability.weekly ?? expert.availability.weekly ?? {};
    const slotError = validateWeeklySlots(nextWeekly);
    if (slotError) {
      return res.status(400).json({ success: false, message: slotError });
    }

    const hasSlots = hasBookableSlots(nextWeekly);
    const incomingMeetingLink = newAvailability.defaultMeetingLink !== undefined
      ? (typeof newAvailability.defaultMeetingLink === 'string' ? newAvailability.defaultMeetingLink.trim() : '')
      : (expert.availability.defaultMeetingLink || '');

    if (hasSlots && !incomingMeetingLink) {
      return res.status(400).json({ success: false, message: "Add a Google Meet link before saving bookable time slots." });
    }
    if (incomingMeetingLink && !/^https?:\/\/meet\.google\.com\/[a-z0-9-]+/i.test(incomingMeetingLink)) {
    ...
    expert.availability.weekly = nextWeekly;
  ...
  try {
    ...
    const formatted = await Promise.all(experts.map(async expert => {
      ...
      const pricing = await getDisplayPrice({
        categoryName: expert.personalInformation?.category || "IT",
        level: expert.professionalDetails?.level || "Intermediate",
        duration: 30
      });
