...
import { getDisplayPrice } from '../utils/pricing.js';

const getSessionDurationMinutes = (session) => {
    if (Number(session?.duration) > 0) return Number(session.duration);
    if (session?.startTime && session?.endTime) {
        return Math.max(1, Math.round((new Date(session.endTime) - new Date(session.startTime)) / 60000));
    }
    return 30;
};

const resolveExpertUser = async (session, fallbackExpertId, User) => {
    const rawExpertId = session?.expertId || fallbackExpertId;
    if (!rawExpertId) return null;

    if (typeof rawExpertId === 'string' && rawExpertId.includes('@')) {
        return User.findOne({ email: rawExpertId.toLowerCase() });
    }

    const directUser = await User.findById(rawExpertId).catch(() => null);
    if (directUser) return directUser;

    const expertProfile = await ExpertDetails.findOne({
        $or: [{ _id: rawExpertId }, { userId: rawExpertId }]
    }).catch(() => null);

    if (!expertProfile?.userId) return null;
    return User.findById(expertProfile.userId).catch(() => null);
};

const resolveSessionPayoutAmount = async (session) => {
    const storedPayout = Number(session?.payoutAmount || 0);
    if (storedPayout > 0) return storedPayout;

    const storedPrice = Number(session?.price || 0);
    if (storedPrice > 0) return storedPrice;

    const expertProfile = await ExpertDetails.findOne({
        $or: [{ _id: session.expertId }, { userId: session.expertId }]
    }).lean().catch(() => null);

    if (!expertProfile) return 0;

    const pricing = await getDisplayPrice({
        categoryName: expertProfile.personalInformation?.category || "IT",
        level: expertProfile.professionalDetails?.level || "Intermediate",
...
const isSessionExpertUser = async (session, userId) => {
    if (!session || !userId) return false;
    const sessionExpertId = String(session.expertId || "");
    const currentUserId = String(userId);

    if (sessionExpertId === currentUserId) return true;

    const expertProfile = await ExpertDetails.findOne({
        $or: [{ _id: session.expertId }, { userId: session.expertId }]
    }).lean().catch(() => null);

    return String(expertProfile?.userId || "") === currentUserId || String(expertProfile?._id || "") === currentUserId;
};
...
export const updateMeetingLink = async (req, res) => {
    try {
        ...
        if (userType !== 'expert' || !(await isSessionExpertUser(session, currentUserId))) {
            return res.status(403).json({ message: "Only the booked expert can update the Google Meet link." });
        }
        if (!/^https?:\/\/meet\.google\.com\/[a-z0-9-]+/i.test(meetingLink)) {
            return res.status(400).json({ message: "Enter a valid Google Meet link (https://meet.google.com/xxx-xxxx-xxx)." });
        }
        const updated = await sessionService.updateSessionMeetingLink(sessionId, meetingLink);
        return res.json({ success: true, message: "Google Meet link updated", data: updated });
    } catch (error) {
...
export const submitReview = async (req, res) => {
    try {
        ...
        if (session) {
            ...
            const [expertReview, candidateReview] = await Promise.all([
                ReviewDetails.findOne({ sessionId, reviewerRole: 'expert' }),
                ReviewDetails.findOne({ sessionId, reviewerRole: 'candidate' })
            ]);

            if (expertReview && candidateReview) {
                try {
                    const payoutAmount = await resolveSessionPayoutAmount(session);
                    
                    // Service charge percentage (20%)
                    const SERVICE_CHARGE_PERCENT = parseFloat(process.env.SERVICE_CHARGE_PERCENT || '20');
                    const serviceCharge = (payoutAmount * SERVICE_CHARGE_PERCENT) / 100;
                    const expertShare = payoutAmount - serviceCharge; // 80% to expert
                    
                    if (!session.payoutCredited && payoutAmount > 0) {
                        const expertUser = await resolveExpertUser(session, expertId, User);

                        if (expertUser) {
                            // Credit only expert's share (80%) to wallet
                            expertUser.walletBalance = Number(expertUser.walletBalance || 0) + expertShare;
                            ...
                            session.expertShare = expertShare;
                            if (Number(session.price || 0) <= 0) session.price = payoutAmount;
                            await session.save();
                            ...
                            // Trigger automatic payout to bank account
                            const { payoutToExpert } = await import('../controllers/paymentController.js');
                            const payoutResult = await payoutToExpert(expertUser._id, payoutAmount, session.sessionId);
                            
                            if (payoutResult.success) {
                                console.log(`💰 Auto-payout completed: ₹${payoutResult.amount} to expert`);
                                walletCreditResult.payoutId = payoutResult.payoutId;
                                walletCreditResult.payoutStatus = payoutResult.status;
                            } else {
                                console.warn(`⚠️ Auto-payout failed: ${payoutResult.reason}. Amount still credited to wallet.`);
                                walletCreditResult.payoutFailed = true;
                                walletCreditResult.payoutReason = payoutResult.reason;
                            }
                            ...
                            await createNotification({
                                ...
                                message: `₹${expertShare} (80% of ₹${payoutAmount}) has been credited to your wallet for session ${session.sessionId}. Service charge: ₹${serviceCharge}`,
                                metadata: {
                ...
                walletCreditResult = {
                    credited: false,
                    amount: Number(session.payoutAmount || session.price || 0),
                    reason: 'Waiting for both expert and candidate feedback'
                };
            }
